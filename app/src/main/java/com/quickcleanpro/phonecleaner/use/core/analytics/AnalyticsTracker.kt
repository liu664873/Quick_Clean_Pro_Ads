...
object AnalyticsTracker {
    ...
    fun track(eventName: String, params: Map<String, Any?> = emptyMap()) {
        runCatching {
            AdvertiseSdk.logEvent(eventName, params.clean())
        }.onFailure { throwable ->
            Log.w(TAG, "track failed: $eventName", throwable)
        }
    }
    ...
    fun setSuperProperties(properties: Map<String, Any?>) {
        runCatching {
            ...
            Log.w(TAG, "set super properties failed", throwable)
        }
    }
    ...
    fun featureForPrimaryRoute(route: String?): FeatureKey? {
        ...
        return FeatureCatalog.byKey.values.firstOrNull { spec -> spec.route.value == route }?.key
    }

    fun homepageReferrer(previousRoute: String?): String? =
        when {
            previousRoute == AppRoute.OnboardingScan.value -> Referrer.GUIDEPAGE
    ...
    private fun writeOnceUserProperties(firstOpenTime: String) {
        ...
        val deviceWritten = setUserOnceAttr(UserProperty.DEVICE_ID, AdvertiseSdk.getThinkingDeviceId())
    ...
    private fun setUserOnceAttr(key: String, value: String): Boolean {
        ...
        return runCatching { AdvertiseSdk.setUserOnceAttr(key, value) }
            .onFailure { throwable -> Log.w(TAG, "set user once failed: $key", throwable) }
            .isSuccess
    }

    private fun setUserAttr(key: String, value: Any) {
        runCatching { AdvertiseSdk.setUserAttr(key, value) }
            .onFailure { throwable -> Log.w(TAG, "set user attr failed: $key", throwable) }
    }

    private fun addUserAttr(key: String, value: Number) {
        runCatching { ThinkingAttr.setUserAddAttr(key, value) }
            .onFailure { throwable -> Log.w(TAG, "add user attr failed: $key", throwable) }
    }

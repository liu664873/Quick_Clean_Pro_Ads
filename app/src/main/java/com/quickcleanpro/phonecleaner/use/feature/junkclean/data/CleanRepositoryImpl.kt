...
class CleanRepositoryImpl(
    ...
    override suspend fun deleteFileItems(
        context: Context,
        selectedItems: List<CleanItem>,
    ): List<JunkDeleteOutcome> =
        withContext(Dispatchers.IO) {
            selectedItems.map { JunkFileDeleteHelper.delete(context, it.junkFile) }
        }

    override fun collectDeleteAuthorizationUris(outcomes: List<JunkDeleteOutcome>): List<Uri> =
        JunkFileDeleteHelper.collectAuthorizationUris(outcomes)

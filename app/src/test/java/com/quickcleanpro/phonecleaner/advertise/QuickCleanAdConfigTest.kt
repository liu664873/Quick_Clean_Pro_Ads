...
class QuickCleanAdConfigTest {
    ...
    fun rawAdPolicyContainsOnlyDeclaredOpenBannerAndInterstitialKeys() {
        val rawKeys = areaKeysFrom("src/main/res/raw/ad_policy.json")
        val expectedKeys =
    ...
    fun rawPoliciesDoNotContainRemovedDraftKeys() {
        val rawKeys =
            areaKeysFrom("src/main/res/raw/ad_policy.json") +
    ...
    fun recallTargetFeatureRoutesProduceEntryAdDecisions() {
        ...
        val expected =
            listOf(
                AppRoute.JunkClean.value to FeatureKey.JUNK_CLEAN,
                AppRoute.BatteryInfo.value to FeatureKey.BATTERY_INFO,
                AppRoute.NetworkSpeed.value to FeatureKey.NETWORK_SPEED,
        )
    ...
    fun everyCatalogFeatureHasEntryAndReturnInterstitialAreaKey() {
        FeatureCatalog.specs.forEach { spec ->
            val entry = AdPlacementRegistry.interstitialArea(AdScene.EnterFeature(spec.key, spec.route.value))
    ...
    private fun rawPolicyRateFor(areaKey: String): Int {
        val content = sourceFile("src/main/res/raw/ad_policy.json").readText()
        val unit =
            AD_UNIT_REGEX
                .findAll(content)
